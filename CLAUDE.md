...
- **Sections with more than one timestamp (2026-08-13).** A section can carry up to 3 moments, and three bare times (`00:34 02:15 07:41`) gave a manager nothing to choose between without hovering each one. So:
  ...
  - **Chips two or more passes independently landed on are marked `×2` / `×3`.** `mergeClips` always counted this (it's how it ranks when a section is over the 3-clip cap) and then dropped it before rendering, so a moment all 3 passes flagged looked identical to one a single pass mentioned. The count says *where to start listening is a better bet*, *not* that the score is more right — the footnote says so.
  - **The badge shows a count, never a fraction.** A degraded grade (V11) can run on fewer than 3 passes, so a printed denominator would sometimes be a lie.
...
- **Columns T/U/V of the Grading Log are the front end's, not Code.gs's.** Follow-up state (`Follow-Up Status` / `Follow-Up Note` / `Follow-Up Date`) is written onto the **AI row** by `index.html` alone. Code.gs appends 19 columns (A–S) with `appendRow` and never reads or writes past S, so the Follow-Ups tab needed **no backend change and no redeploy** — and mustn't be given one by accident. Two things to keep in step: `ensureExtendedHeaders` in Code.gs only writes headers when `Source` is missing (so extra columns don't confuse it), and deleting a submission clears `A:V`, not `A:S`, or the Follow-Ups tab keeps a ghost row.
...
- **Code.gs needs a redeploy for corrections (V13) to appear.** The front end renders the block already, but no session has a fix-list until the new prompt is live — until then the block simply doesn't show, on new and old sessions alike. Same redeploy also carries the Langley phone number below.
